...
import { X, PlayCircle, FileText, CheckCircle2, ChevronRight, Award, Loader2, AlertCircle, XCircle, Clock } from 'lucide-react'
import ModalAlerta from '@/src/components/shared/ModalAlerta' // <--- IMPORTAMOS TU MODAL
...
interface VisorProps {
  ...
  onGetDetalles: (cursoId: number) => Promise<any>;
}
...
export default function VisorCurso({ curso, sessionUserId, onClose, onSubmitExamen, onGuardarProgreso, onGetDetalles }: VisorProps) {
  ...
  const [detallesRevision, setDetallesRevision] = useState<any>(null)
    ...
    if (miAsignacion) {
      if (miAsignacion.estado === 'completado') {
        ...
        try {
          ...
          if (draft.fase === 'examen' && curso.tiempo_limite_examen > 0) {
            const inicio = new Date(draft.tiempoInicio).getTime();
            const limite = curso.tiempo_limite_examen * 60 * 1000;
            const ahora = new Date().getTime();
            const transcurrido = ahora - inicio;
            const restante = Math.max(0, Math.floor((limite - transcurrido) / 1000));
            setTiempoRestante(restante);
            setTiempoInicio(draft.tiempoInicio);

            if (restante === 0) {
               handleEnviarForzado(draft.respuestas, draft.tiempoInicio);
            }
          }
        } catch(e) {}
      }
  ...
  useEffect(() => {
    if (fase === 'examen' && tiempoRestante !== null && tiempoRestante > 0 && !loading) {
      timerRef.current = setInterval(() => {
        setTiempoRestante(prev => {
          if (prev && prev <= 1) {
            clearInterval(timerRef.current!);
            handleEnviarForzado(respuestas, tiempoInicio);
            return 0;
          }
          return (prev || 0) - 1;
        });
  ...
  }, [fase, tiempoRestante, loading]);


  const avanzarAExamen = () => {
    setFase('examen')
    const tInicio = new Date().toISOString();
    setTiempoInicio(tInicio);
    
    if (curso.tiempo_limite_examen > 0) {
      setTiempoRestante(curso.tiempo_limite_examen * 60); 
    }

    onGuardarProgreso(curso.id, { fase: 'examen', respuestas, tiempoInicio: tInicio })
  }
  ...
  const handleSeleccionarOpcion = (preguntaId: number, opcionId: number) => {
    ...
    onGuardarProgreso(curso.id, { fase: 'examen', respuestas: nuevasRespuestas, tiempoInicio })
  }
  ...
  const handleEnviar = async () => {
    ...
    if (Object.keys(respuestas).length < totalPreguntas) {
      ...
      mostrarAlerta("Preguntas sin responder", "Por favor, responde todas las preguntas antes de enviar tu evaluación.", "warning");
      return
    }
    await procesarEnvio(respuestas, tiempoInicio);
  }

  const handleEnviarForzado = async (respuestasActuales: any, inicio: string) => {
    // Reemplazamos el alert() nativo por una alerta informativa 👇
    mostrarAlerta(
      "Tiempo agotado", 
      "¡El tiempo del examen ha finalizado! Tus respuestas se están enviando automáticamente.", 
      "info"
    );
    await procesarEnvio(respuestasActuales, inicio);
  }

  const procesarEnvio = async (respuestasAEnviar: any, inicio: string) => {
    setLoading(true)
    if (timerRef.current) clearInterval(timerRef.current); 
    try {
      const formatoRespuestas = Object.entries(respuestasAEnviar).map(([pId, oId]) => ({
        pregunta_id: parseInt(pId), opcion_elegida_id: oId as number
      }))

      await onSubmitExamen(curso.id, formatoRespuestas, inicio)
      const detalle = await onGetDetalles(curso.id)
      setResultadoFinal(detalle)
      setFase('resultados')
    } catch (error: any) {
      // Reemplazamos el alert() nativo de error 👇
  ...
  return (
    <>
      <div className="fixed inset-0 z-[100] bg-neutral-100/95 dark:bg-neutral-950/95 backdrop-blur-xl flex flex-col animate-in fade-in duration-300">
        ...
        <div className="h-16 sm:h-20 px-4 sm:px-8 bg-white/50 dark:bg-black/50 border-b border-neutral-200/50 dark:border-neutral-800/50 flex items-center justify-between shrink-0 shadow-sm z-10 backdrop-blur-md">
          ...
          <div className="flex items-center gap-4">
            {/* RELOJ TEMPORIZADOR VISUAL */}
            {fase === 'examen' && tiempoRestante !== null && (
              <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border-2 font-black tracking-widest transition-colors ${
                tiempoRestante <= 60 
                ...
                <Clock size={18} className={tiempoRestante <= 60 ? 'text-red-500' : 'text-neutral-400'} />
              ...
              <div className="animate-in slide-in-from-bottom-8 duration-500">
                ...
                <div className="mt-8 bg-white dark:bg-neutral-900 rounded-[2rem] p-6 sm:p-8 flex items-center justify-between shadow-lg">
                  <div>
                    ...
                    {curso.tiempo_limite_examen > 0 && (
                      <p className="text-orange-600 dark:text-orange-400 text-sm font-bold flex items-center gap-1.5 mt-2">
                        <AlertCircle size={16} /> El examen tiene un límite de tiempo de {curso.tiempo_limite_examen} minutos.
                      </p>
                    )}
                  </div>
                ...
                {resultadoFinal ? (
                  <>
                    ...
                    <div className="bg-white dark:bg-neutral-900 p-8 rounded-[2rem] border border-neutral-200 dark:border-neutral-800 shadow-xl w-full max-w-sm text-center mb-10">
                      ...
                      <p className={`text-6xl font-black ${resultadoFinal.aprobado ? 'text-emerald-500' : 'text-rose-500'}`}>{Math.round(resultadoFinal.calificacion)}<span className="text-3xl">%</span></p>
                    </div>
                    ...
                    <div key={pregunta.id} className="bg-white dark:bg-neutral-900 p-6 sm:p-8 rounded-[2rem] shadow-sm border border-neutral-200 dark:border-neutral-800 relative overflow-hidden">
                      ...
                      <div className="space-y-3 pl-4 sm:pl-9">
                        {pregunta.opciones_pregunta?.map((opcion: any) => {
                          ...
                          let colorClass = 'border-neutral-200 dark:border-neutral-800 opacity-60'; 
                          if (isSelected && isCorrectAnswer) colorClass = 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-900 dark:text-emerald-100 scale-[1.01] opacity-100';
                          if (isSelected && !isCorrectAnswer) colorClass = 'border-rose-500 bg-rose-50 dark:bg-rose-900/20 text-rose-900 dark:text-rose-100 scale-[1.01] opacity-100'; 
                          if (!isSelected && isCorrectAnswer) colorClass = 'border-emerald-500/50 text-emerald-600 dark:text-emerald-400 opacity-100 border-dashed'; 
      ...
      {/* RENDERIZADO DEL MODAL DE ALERTA PERSONALIZADO 👇 */}
      <ModalAlerta 
