...
import {
  ...
  Newspaper,
  Timer,
  GraduationCap // <--- Ícono para Capacitación
} from "lucide-react";
...
interface MenuItem {
  ...
  submenu?: SubMenuItem[];
  permission?: string | string[];
  isCapacitacion?: boolean; // Propiedad especial para la validación dinámica
}
...
export default function Sidebar({ permissions = [] }: SidebarProps) {
  ...
  // Estados para validar si el empleado tiene cursos asignados
  const [hasAssignedCourses, setHasAssignedCourses] = useState(false);
  
  // Validamos si es Admin de cursos (puede crear o tiene acceso total)
  ...
  /* ================= EFECTO PARA VALIDAR CURSOS ASIGNADOS ================= */
  useEffect(() => {
    let isMounted = true;

    const checkCourses = async () => {
      // Si ya tiene permisos de administrador de cursos, no es necesario hacer la consulta
      ...
      if (emp && isMounted) {
        // Consultar si tiene al menos 1 curso asignado
        const { count } = await supabase
          .from('asignacion_cursos')
          .select('*', { count: 'exact', head: true })
          .eq('empleado_id', emp.id);
          
        if (count && count > 0 && isMounted) {
  ...
  const allPaths = useMemo(() => [
    ...
    "/dashboard/capacitacion", // <-- Añadido al prefetch
  ...
  const getActiveBaseColor = (path: string) => {
    if (path.includes("/dashboard/capacitacion")) return "text-rose-600 dark:text-rose-400";
  ...
  const getActiveStyles = (id: string) => {
    if (id === "capacitacion") return "bg-rose-500/15 text-rose-600 dark:bg-rose-600/25 dark:text-rose-400";
  ...
  const rawMenuItems: MenuItem[] = [
    ...
    },
    {
      ...
      submenu: [
        ...
        { icon: <BarChart3 size={18} />, label: "Reportes" , path: "/dashboard/rendimiento/reportes", permission: ["reportes.read_all", "acceso_total", "actividades.read"] },
      ],
    },
    /* === NUEVO MÓDULO DE CAPACITACIÓN === */
    {
      id: "capacitacion",
      icon: <GraduationCap size={20} />,
      label: "Capacitación",
      path: "/dashboard/capacitacion",
      isCapacitacion: true, // Usado para la validación dinámica
    },
    {
      id: "nomina",
  ...
  const filteredMenuItems = useMemo(() => {
    ...
    return rawMenuItems.reduce((acc, item) => {
      // 1. VALIDACIÓN ESPECIAL PARA CAPACITACIÓN
      if (item.isCapacitacion) {
        // Se muestra SI es administrador de cursos O SI tiene cursos asignados
        if (!canManageCourses && !hasAssignedCourses) return acc;
      } 
      // 2. VALIDACIÓN NORMAL PARA EL RESTO
