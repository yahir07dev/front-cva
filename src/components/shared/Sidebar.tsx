...
export default function Sidebar({}: SidebarProps) {
  ...
  // Solo necesitamos toggleTheme, ya no usamos la variable 'theme' para estilos
  ...
  // REFACTORIZACIÓN: Ahora devolvemos una sola cadena con clases Light y Dark juntas
  const getModuleClasses = (moduleId: string) => {
    switch (moduleId) {
      case 'rendimiento':
        return {
          active: 'bg-orange-50 text-orange-600 dark:bg-orange-500/10 dark:text-orange-400',
      ...
      default:
        return {
          active: 'bg-gray-50 text-gray-600 dark:bg-gray-800 dark:text-gray-300',
  ...
  // Clases comunes para ítems inactivos (reutilizables)
  const inactiveClasses = "text-gray-600 hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-[#2d3142] dark:hover:text-white"
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed top-5 left-4 z-[60] md:hidden transition-opacity duration-300 
      ...
      `}>
        ...
        <div className={`h-20 flex items-center px-5 relative shrink-0 transition-all duration-300 ${collapsed ? 'justify-center' : 'justify-between'}`}>
          <div className={`flex items-center gap-3 overflow-hidden transition-all duration-300 ${collapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'}`}>
            <button onClick={() => router.push('/dashboard')} className="flex items-center gap-3 group">
              ...
              <span className="font-bold text-lg tracking-tight whitespace-nowrap text-gray-900 dark:text-white">RRHH</span>
            </button>
          </div>
          ...
          <button 
            ...
            className={`
              ...
              dark:text-gray-500 dark:hover:text-white dark:hover:bg-gray-800
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto scrollbar-none">
          {menuItems.map((item, index) => {
            const itemClasses = getModuleClasses(item.id)
              ...
              <div key={index}>
                <div className="mb-2">
                  <button 
                    onClick={() => { 
                      if (item.hasSubmenu) { 
                        if (collapsed) { setCollapsed(false); setRendimientoOpen(true); } 
                        else { setRendimientoOpen(!rendimientoOpen); } 
                    ...
                    className={`
                      ...
                      ${isItemActive 
                        ? `${itemClasses.active} font-medium` 
                        : inactiveClasses
                      }
                    `} 
                  ...
                  >
                    ...
                    {item.hasSubmenu && (
                      <ChevronRight size={16} className={`transition-all duration-300 text-gray-400 dark:text-gray-500 ${rendimientoOpen ? 'rotate-90' : ''} ${collapsed ? 'hidden' : 'block'}`} />
                    )}
                  </button>
                  ...
                  {item.hasSubmenu && rendimientoOpen && (
                    <div className={`
                      mt-1 ml-4 pl-4 border-l-2 space-y-1 overflow-hidden transition-all duration-300 
                      border-gray-200 dark:border-[#2d3142]
        ...
        <div className="p-4 space-y-2 mt-auto shrink-0">
          ...
          >
            <span className={`min-w-[24px] flex justify-center transition-transform duration-300 ${collapsed ? 'scale-110' : ''}`}>
               {/* Icono dinámico CSS puro: Sun se oculta en dark, Moon se muestra en dark */}
