...
import { Pencil, Trash, Search, ShieldAlert } from "lucide-react";
import { useRouter } from "next/navigation";
import ConfirmDialog from "../shared/ConfirmDialog";
import SuccessDialog from "../shared/SuccessDialog";
import { darDeBajaEmpleadoAction } from "@/src/actions/personal/empleadosActions";
...
export default function EmpleadosTable({ initialEmpleados }: EmpleadosTableProps) {
  ...
  const filteredEmpleados = empleados.filter((e) => {
    const fullSearch = `${e.nombre} ${e.apellidos} ${e.rol?.nombre || ""} ${e.area?.nombre || ""}`.toLowerCase();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await darDeBajaEmpleadoAction(empleadoSeleccionado.id.toString());
      setShowSuccess(true);
      setEmpleados((prev) => prev.filter((e) => e.id !== empleadoSeleccionado.id));
    ...
    <div className="w-full space-y-8 pb-24 animate-in fade-in duration-700">
      ...
      <div className="relative max-w-3xl mx-auto group">
        ...
        <input
          ...
          className="w-full pl-14 pr-6 py-4 rounded-3xl bg-white/80 dark:bg-neutral-900/80 backdrop-blur-xl border border-neutral-200/60 dark:border-neutral-800/60 shadow-sm hover:shadow-lg focus:shadow-xl outline-none transition-all duration-300 text-base font-medium text-neutral-900 dark:text-white placeholder:text-neutral-500/70 focus:border-indigo-500/60 focus:ring-4 focus:ring-indigo-500/15"
        />
      </div>
