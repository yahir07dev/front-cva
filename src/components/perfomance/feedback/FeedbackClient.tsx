...
export default function FeedbackClient({ 
  ...
  const {
    ...
    stats,
    scrollRef
  } = useFeedback({ initialEmpleados, userId, canManage, userEstado,
    canCreate })

  const [mensajeIdParaBorrar, setMensajeIdParaBorrar] = useState<number | null>(null)
  const isBaja = userEstado === 'baja'
  
  // EFECTO DE AUTO-SELECCIÓN: Si NO es manager, auto-selecciona su propio perfil
  useEffect(() => {
    if (!canManage && !selectedEmp && userId) {
      const me = empleados.find(e => e.usuario_id === userId)
      if (me) setSelectedEmp(me)
    }
  }, [canManage, selectedEmp, empleados, userId, setSelectedEmp])
  ...
  const MiniStatCard = ({ icon: Icon, label, value, color, bg }: any) => (
    <div className={`min-w-[140px] snap-center flex flex-col items-start justify-center p-3 gap-1.5 rounded-2xl border border-neutral-100 dark:border-0 bg-white dark:bg-white/[0.02] shadow-sm transition-all duration-300 md:min-w-0 md:w-full md:p-5 md:gap-3 md:hover:shadow-md md:hover:-translate-y-1`}>
      <div className="flex items-center gap-3 w-full">
        <div className={`p-1.5 rounded-lg transition-all shrink-0 md:p-3 md:rounded-xl ${bg} ${color}`}>
          <Icon strokeWidth={2.5} className="h-4 w-4 md:h-6 md:w-6" />
        </div>
        <span className={`text-xl font-black truncate transition-all md:text-3xl ${color}`}>{value}</span>
      </div>
      <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider whitespace-nowrap md:text-sm md:text-neutral-500 md:whitespace-normal">{label}</p>
    </div>
  )
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100 overflow-hidden relative">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* ÁREA DE CHAT */}
        <div className={`flex-1 flex flex-col h-full min-w-0 relative bg-neutral-50/50 dark:bg-black/20 ${!canManage ? 'flex' : (selectedEmp ? 'flex' : 'hidden md:flex')} ${isBaja ? 'opacity-80 pointer-events-none' : ''}`}>
          {selectedEmp ? (
            <>
              {/* Header */}
              <div className="flex-none h-14 px-3 flex items-center justify-between bg-white/90 dark:bg-neutral-950/90 backdrop-blur-xl border-b border-neutral-100 dark:border-0 z-20 md:px-6">
                <div className="flex items-center gap-3 min-w-0">
                  {canManage && (
                    <button onClick={() => setSelectedEmp(null)} className="md:hidden p-1.5 -ml-1 text-neutral-500 active:bg-neutral-100 rounded-full transition-colors">
                      <ArrowLeft size={18} />
                    </button>
                  )}
                  {canManage && (
                    <div className="relative h-8 w-8 rounded-full overflow-hidden bg-neutral-200 dark:bg-neutral-800 shrink-0 ring-1 ring-neutral-200 dark:ring-neutral-800">
                      {selectedEmp.foto_perfil_url ? (
                          <img src={selectedEmp.foto_perfil_url} alt={selectedEmp.nombre} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                      ) : (
                          <div className="h-full w-full flex items-center justify-center text-[10px] font-bold text-neutral-500 uppercase">{selectedEmp.nombre?.[0]}</div>
                      )}
                    </div>
                  )}
              ...
              <div className="flex-none pt-3 pb-2 px-3 bg-neutral-50/50 dark:bg-transparent z-10 md:px-6 md:py-6">
                <div className="flex gap-3 overflow-x-auto scrollbar-hide snap-x pb-2 md:grid md:grid-cols-3 md:gap-6 md:overflow-visible md:pb-0 px-1">
                  <MiniStatCard icon={ListTodo} label="Total Feedback" value={stats.total} color="text-blue-600 dark:text-blue-400" bg="bg-blue-100 dark:bg-blue-500/10" />
                  <MiniStatCard icon={CheckCircle2} label="Positivos" value={stats.positivos} color="text-emerald-600 dark:text-emerald-400" bg="bg-emerald-100 dark:bg-emerald-500/10" />
                  <MiniStatCard icon={TrendingUp} label="A Mejorar" value={stats.mejora} color="text-orange-600 dark:text-orange-400" bg="bg-orange-100 dark:bg-orange-500/10" />
                </div>
              </div>

              {/* LISTA MENSAJES */}
              <div className="flex-1 min-h-0 overflow-y-auto px-2 sm:px-4 pb-4 scrollbar-thin scrollbar-thumb-neutral-300 dark:scrollbar-thumb-neutral-700 md:px-6" ref={scrollRef}>
              ...
              {canCreate && !isBaja && (
                <div className="flex-none z-30 w-full bg-white dark:bg-neutral-950 border-t border-neutral-100 dark:border-0 pb-safe">
                  <ChatInput form={form} setForm={setForm} onSend={handleSend} loading={loading} />
                </div>
              )}
            </>
          ) : (
