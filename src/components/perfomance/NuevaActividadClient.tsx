...
export default function NuevaActividadClient() {
  ...
  return (
    <div className="flex flex-col h-[100dvh] w-full overflow-hidden bg-neutral-50 dark:bg-neutral-950 transition-colors duration-500 font-sans">
      ...
      {/* 1. Header Fijo - CORREGIDO: z-50 bajado a z-20 para que no tape el Sidebar */}
      <header className="flex-none sticky top-0 z-20 bg-neutral-50/80 dark:bg-neutral-950/80 backdrop-blur-md border-b border-neutral-200/50 dark:border-0 px-4 py-4 sm:px-8">
        <div className="mx-auto max-w-5xl flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button 
              onClick={() => router.back()}
              className="flex h-10 w-10 items-center justify-center rounded-full bg-white dark:bg-white/5 border border-neutral-200 dark:border-0 transition-all active:scale-90"
            >
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-neutral-900 dark:bg-white text-white dark:text-black text-sm font-bold transition-all active:scale-95 disabled:opacity-30 shadow-xl shadow-black/10 dark:shadow-none"
          >
      ...
      <main className="flex-1 overflow-y-auto px-4 py-8 sm:px-6 lg:px-10 pb-40 scrollbar-thin scrollbar-thumb-neutral-300 dark:scrollbar-thumb-neutral-800 hover:scrollbar-thumb-neutral-400 dark:hover:scrollbar-thumb-neutral-700">
        <div className="mx-auto max-w-5xl space-y-8">
          ...
          {isBaja && (
            <div className="flex items-center gap-4 p-5 rounded-[24px] bg-rose-500/5 border border-rose-500/10 text-rose-600 dark:text-rose-400">
          ...
          <div className={`grid gap-8 lg:grid-cols-3 transition-opacity duration-500 ${isBaja ? 'opacity-40 grayscale pointer-events-none' : 'opacity-100'}`}>
            ...
            <aside className="space-y-8">
              <section className="bg-white/40 dark:bg-transparent backdrop-blur-md rounded-[32px] p-6 border border-neutral-200/50 dark:border-0">
                ...
                <div className="space-y-8">
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        { id: 'baja', label: 'Baja', icon: CircleDot, active: 'bg-emerald-500/10 text-emerald-600' },
                        { id: 'media', label: 'Media', icon: AlertCircle, active: 'bg-amber-500/10 text-amber-600' },
                        { id: 'alta', label: 'Alta', icon: Zap, active: 'bg-rose-500/10 text-rose-600' }
                      ].map((p) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex flex-col items-center gap-2 py-3 rounded-2xl text-[10px] font-bold transition-all border ${isActive ? `${p.active} border-transparent shadow-sm scale-105` : 'bg-neutral-100 dark:bg-white/5 border-transparent text-neutral-400'}`}
                          >
                  ...
                  <div className="p-4 rounded-2xl bg-orange-500/5 border border-orange-500/10 flex gap-3">
                    ...
                    <p className="text-[10px] font-medium text-orange-600 dark:text-orange-400/80 leading-relaxed uppercase tracking-tighter">
                      Vencimiento automático al expirar.
                    </p>
                  </div>
                </div>
              </section>
