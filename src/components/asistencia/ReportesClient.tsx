    ...
    <div ref={containerRef} className="relative w-full">
      <div
        className={`min-h-12 w-full flex flex-wrap items-center gap-2 px-4 py-2 rounded-2xl border-2 transition-all duration-200 cursor-text ${
          open
            ? "border-fuchsia-500 ring-4 ring-fuchsia-500/10 bg-white dark:bg-neutral-900"
      ...
      >
        ...
        {todosActivos ? (
          ...
          empleados
            ...
            .map((e) => (
              <span
                key={e.id}
                className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-fuchsia-600 text-white text-[11px] font-bold uppercase tracking-wider"
              >
                {e.nombre}
                <button
                  onClick={(ev) => {
                  ...
                  className="hover:text-fuchsia-200 cursor-pointer"
                >
                  ×
                </button>
              </span>
        ...
        <input
          ...
          className="flex-1 min-w-[60px] text-sm outline-none bg-transparent text-neutral-900 dark:text-white"
        />
      </div>
      {open && (
        <div className="absolute z-50 mt-2 w-full bg-white dark:bg-neutral-900 border-2 border-neutral-100 dark:border-neutral-800 rounded-3xl shadow-2xl overflow-hidden">
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-bold transition-colors cursor-pointer ${todosActivos ? "bg-fuchsia-600 text-white" : "text-neutral-700 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-neutral-800"}`}
          >
            <div
              className={`w-4 h-4 rounded border-2 ${todosActivos ? "border-white" : "border-neutral-300 dark:border-neutral-600"}`}
            />
            TODOS LOS EMPLEADOS
          </button>
            ...
            {filtrados.map((emp) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 text-sm cursor-pointer transition-colors ${selected ? "bg-fuchsia-50 dark:bg-fuchsia-900/20 text-fuchsia-600 dark:text-fuchsia-400 font-bold" : "text-neutral-600 dark:text-neutral-400 hover:bg-neutral-50 dark:hover:bg-neutral-800"}`}
                >
                  <div
                    className={`w-4 h-4 rounded border-2 transition-colors ${selected ? "border-fuchsia-600 bg-fuchsia-600" : "border-neutral-300 dark:border-neutral-600"}`}
                  />
...
export default function ReportesClient({
  ...
  const generarReporte = async () => {
    ...
    try {
      ...
      const data = await getRegistrosAsistenciaAction(
        fechaInicio,
        fechaFin,
        tipoFiltro,
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <AlertDialog
        open={showErrorModal}
        title="Faltan datos"
        description="Por favor, selecciona una fecha de inicio y una fecha de fin."
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-1">
              ...
              <p className="text-neutral-500 dark:text-neutral-400 font-medium">
                Control de puntualidad e historial.
              </p>
            </div>
            {hasSearched && reporte.length > 0 && (
              <button
                ...
                className={`flex items-center justify-center gap-3 px-6 py-3.5 rounded-2xl font-black text-sm transition-all shadow-xl ${isExporting ? "bg-neutral-300 dark:bg-neutral-800 text-neutral-500 cursor-not-allowed" : "bg-red-500 text-white hover:bg-red-600 hover:scale-105 active:scale-95"}`}
              >
                {isExporting ? (
                  <>
          ...
          {hasSearched && (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="bg-white dark:bg-neutral-900 p-6 rounded-[2rem] border-2 border-neutral-100 dark:border-neutral-800/60 flex items-center gap-5 shadow-sm">
                <div className="w-14 h-14 bg-blue-50 dark:bg-fuchsia-500/10 rounded-2xl flex items-center justify-center text-blue-600 dark:text-blue-400">
              ...
              <div className="bg-white dark:bg-neutral-900 p-6 rounded-[2rem] border-2 border-neutral-100 dark:border-neutral-800/60 flex items-center gap-5 shadow-sm">
                <div className="w-14 h-14 bg-green-50 dark:bg-fuchsia-500/10 rounded-2xl flex items-center justify-center text-green-600 dark:text-green-400">
              ...
              <div className="bg-white dark:bg-neutral-900 p-6 rounded-[2rem] border-2 border-neutral-100 dark:border-neutral-800/60 flex items-center gap-5 shadow-sm">
                <div className="w-14 h-14 bg-neutral-100 dark:bg-neutral-800 rounded-2xl flex items-center justify-center text-neutral-600 dark:text-neutral-400">
              ...
              <div className="bg-white dark:bg-neutral-900 p-6 rounded-[2rem] border-2 border-neutral-100 dark:border-neutral-800/60 flex items-center gap-5 shadow-sm">
                <div className="w-14 h-14 bg-pink-50 dark:bg-pink-500/10 rounded-2xl flex items-center justify-center text-pink-600 dark:text-pink-400">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-end">
                ...
                <div className="flex p-1.5 bg-neutral-100 dark:bg-black/20 rounded-[1.25rem] border-2 border-neutral-200/50 dark:border-neutral-800">
                  {(["asistencia", "comida"] as const).map((t) => (
                    <button
                      ...
                      className={`flex-1 py-2.5 text-xs font-black rounded-xl transition-all cursor-pointer ${tipoFiltro === t ? "bg-white dark:bg-neutral-800 text-fuchsia-600 dark:text-fuchsia-400 shadow-md" : "text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300"}`}
                    >
                      {t.toUpperCase()}
                    </button>
                  ))}
              ...
              <div className="lg:col-span-3 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input
                    ...
                    className="w-full bg-white dark:bg-neutral-900 border-2 border-neutral-200 dark:border-neutral-800 rounded-xl px-4 py-2.5 text-sm outline-none cursor-pointer text-neutral-900 dark:text-white [color-scheme:light] dark:[color-scheme:dark]"
                  />
                  <input
                    type="date"
                    value={fechaFin}
              ...
              <div className="lg:col-span-2">
                <button
                  ...
                  disabled={loading}
                  className="w-full py-4 bg-fuchsia-600 text-white rounded-[1.25rem] font-black text-sm hover:bg-fuchsia-500 shadow-lg shadow-fuchsia-500/20 cursor-pointer disabled:opacity-50 transition-all active:scale-95 flex items-center justify-center"
                >
                  {loading ? (
              ...
              <table className="w-full min-w-[800px]">
                <thead>
                  <tr className="bg-neutral-50 dark:bg-black/20 border-b-2 border-neutral-100 dark:border-neutral-800/60">
                    {[
                      ...
                      <th
                        key={h}
                        className="px-6 py-5 text-left text-[10px] font-black text-neutral-400 uppercase tracking-widest"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-neutral-50 dark:divide-neutral-800/50">
                  {reporte.map((fila, i) => (
                    <tr
                      key={i}
                      className={`hover:bg-neutral-50/50 dark:hover:bg-white/[0.02] transition-colors ${fila.estatus === "Retardo" || fila.estatus === "Excedido" ? "bg-red-100 dark:bg-red-500/10" : `${fila.estatus === "A tiempo" ? "bg-green-100/40 dark:bg-green-500/10" : ""}`}`}
                    >
                      <td className="px-6 py-4 text-sm font-medium text-neutral-500 dark:text-neutral-400 tabular-nums">
                        {fila.fecha}
                      </td>
                      ...
                      <td className="px-6 py-4 text-sm text-neutral-600 dark:text-neutral-400 tabular-nums">
                      ...
                      <td className="px-6 py-4">
                        <div
                          className={`flex items-center gap-2 font-black tabular-nums ${fila.estatus === "Retardo" || fila.estatus === "Excedido" ? "text-pink-500" : "text-fuchsia-600 dark:text-fuchsia-400"}`}
                        >
