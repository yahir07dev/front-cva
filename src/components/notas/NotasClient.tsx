...
import { Plus, Pin, Trash2, X, Save, Loader2, Search, ChevronLeft, MoreVertical, Share2 } from 'lucide-react'
...
const extensions = [
  ...
  ResizableImageForHTML, ImageExtension,
  TaskList, TaskItem,
]
...
export default function NotasClient() {
  ...
  const openCreateModal = () => {
    setCurrentNota({ titulo: '', contenidoJson: '', colorFondo: BG_COLORS[0], pinned: false })
    setModalOpen(true)
  ...
  const openEditModal = (nota: any) => {
    ...
    setCurrentNota({
      id: nota.id,
      titulo: nota.titulo,
      contenidoJson: nota.contenido_json,
      colorFondo: nota.color_fondo || BG_COLORS[0],
      pinned: nota.pinned
    })
    setModalOpen(true)
    setShowMoreMenu(false)
  ...
  const handleSave = async () => {
    if (!currentNota?.titulo.trim()) return alert('El título es obligatorio')
    setIsSaving(true)
    try {
      if (currentNota.id) {
        await handleUpdate(currentNota.id, currentNota.titulo, currentNota.contenidoJson, currentNota.colorFondo)
      } else {
        await handleCreate(currentNota.titulo, currentNota.contenidoJson, currentNota.colorFondo)
      }
      setModalOpen(false)
    } catch (error: any) {
      alert(error.message)
  ...
  if (!canManage && !canCreate) {
    return (
      <div className="flex h-full items-center justify-center text-neutral-500">
        No tienes permisos para ver las notas.
      </div>
    )
  }
  ...
  return (
    <div className="relative flex flex-col h-full w-full bg-neutral-50 dark:bg-neutral-950 font-sans overflow-hidden">
      ...
      <div className="px-4 pt-6 pb-4 sm:px-8 shrink-0">
        <h1 className="text-2xl sm:text-3xl font-black text-neutral-900 dark:text-white tracking-tight mb-4">
          Notas de Equipo
        </h1>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-white dark:bg-neutral-900 rounded-2xl shadow-sm border border-transparent focus:ring-2 focus:ring-[#25C2FF] outline-none text-sm font-medium transition-all"
          />
        </div>
      </div>

      {/* ── GRID DE CUADRITOS PERFECTOS ─────────────────────────────────── */}
      <div className="flex-1 overflow-y-auto px-4 sm:px-8 pb-28 scrollbar-hide">
        {/*  AQUÍ ESTÁ LA MAGIA: grid-cols fijos en lugar de columns (Masonry)  */}
          ...
          {notasFiltradas.map((nota) => {
            ...
            return (
              <div
                ...
                // aspect-square hace que sean cuadritos exactos, sin importar el contenido 
              ...
              >
                ...
                <div className="relative flex flex-col h-full z-10">
                  ...
                  {/* Estilos para vista previa de checkboxes */}
                  ...
                  {/* 👇 Vista Previa del texto/imagen (Se corta suavemente al final) 👇 */}
                  <div 
                    className="preview-content prose prose-sm flex-1 overflow-hidden pointer-events-none text-xs sm:text-sm" 
                    style={{ 
                      color: cardText, 
                      opacity: 0.8,
                      // Truco de CSS para que el texto/imagen haga un efecto "fade out" si no cabe
                ...
                {canDelete && (
                  <button onClick={e => { e.stopPropagation(); handleDelete(nota.id) }} className="absolute top-3 right-3 p-1.5 bg-black/20 hover:bg-red-500 hover:text-white rounded-full opacity-0 group-hover:opacity-100 transition-all active:scale-90 z-20">
                    <Trash2 size={13} />
                  </button>
                )}
              </div>
      ...
      {/* ── FAB (BOTÓN FLOTANTE CELESTE) ─────────────────────────────────── */}
      {canCreate && (
        <button 
          onClick={openCreateModal} 
          className="absolute bottom-24 right-6 md:bottom-10 md:right-8 bg-[#25C2FF] hover:bg-[#0ea5e9] text-white p-4 rounded-full shadow-[0_8px_30px_rgba(37,194,255,0.45)] transition-transform hover:scale-105 active:scale-95 z-10"
        >
      ...
      {modalOpen && currentNota && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          >
            ...
            <div
              className="relative z-10 flex items-center justify-between px-3 py-2.5"
              style={{ background: 'rgba(0,0,0,0.04)' }} 
            >
              <button onClick={() => setModalOpen(false)} className="p-2 rounded-full hover:bg-[rgba(128,128,128,0.2)] active:scale-95 transition-all">
                <ChevronLeft size={24} color={noteTextColor} />
              </button>

              <div className="flex flex-col items-center" style={{ color: noteSubColor }}>
              ...
              <div className="flex items-center gap-1">
                <button
                  onClick={handleSave}
                  disabled={isSaving}
                  className="flex items-center justify-center bg-[#25C2FF] hover:bg-[#0ea5e9] text-white px-4 py-1.5 rounded-full shadow-md transition-all active:scale-95 disabled:opacity-50"
                  title="Guardar"
                >
                  {isSaving ? <Loader2 size={18} className="animate-spin" /> : <span className="text-sm font-bold">Guardar</span>}
                </button>
                ...
                {currentNota.id && (
                  <button 
                    onClick={async () => {
                      ...
                      setCurrentNota(prev => prev ? { ...prev, pinned: !prev.pinned } : null)
                    }} 
                ...
                {currentNota.id && canDelete && (
                  <div className="relative">
                    <button 
                      ...
                      className="p-2 rounded-full hover:bg-[rgba(128,128,128,0.2)] active:scale-95 transition-all"
                    >
                          ...
                          <button
                            onClick={async () => {
                              if (confirm('¿Estás seguro de eliminar esta nota?')) {
                                await handleDelete(currentNota.id!)
                                setModalOpen(false)
                                setShowMoreMenu(false)
                              }
                            }}
            ...
            <input
              ...
              onChange={e => setCurrentNota({ ...currentNota, titulo: e.target.value })}
              placeholder="Título"
            ...
            <div className="relative z-10 flex-1 overflow-hidden flex flex-col">
              <NotaEditor
                ...
                onChange={json => setCurrentNota({ ...currentNota, contenidoJson: json })}
                colorFondo="transparent"
                textColor={noteTextColor}
                onBgColorChange={c => setCurrentNota({ ...currentNota, colorFondo: c })}
              />
            </div>
          </div>
        </div>
