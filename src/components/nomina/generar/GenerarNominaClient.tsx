...
export default function GenerarNominaClient({ canManage }: { canManage: boolean }) {
  ...
  return (
    <div className="w-full mx-auto flex flex-col h-[calc(100vh-8rem)] pb-4 sm:pb-8 relative">
      ...
      <div className="shrink-0 w-full bg-white/80 dark:bg-neutral-900/60 backdrop-blur-xl border border-neutral-200/60 dark:border-neutral-800/50 rounded-[24px] sm:rounded-[32px] shadow-sm p-4 sm:p-5 md:p-6 mb-4 md:mb-6 relative z-30">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-3 w-full lg:w-1/2">
            <div className="relative flex-1 min-w-0">
              <label className="flex items-center gap-1.5 text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-1.5 ml-1">
                <CalendarDays size={14} /> Fecha de Pago
              </label>

              <button
                onClick={() => { setIsDateMenuOpen(!isDateMenuOpen); setIsEmpMenuOpen(false) }}
                className={`w-full flex items-center justify-between px-4 py-3 sm:py-3.5 rounded-2xl text-sm font-semibold transition-all duration-200 bg-neutral-50 dark:bg-neutral-950 border ${isDateMenuOpen ? 'border-emerald-500 ring-2 ring-emerald-500/20' : 'border-neutral-200/60 dark:border-neutral-800/50 hover:border-neutral-300 dark:hover:border-neutral-700'}`}
              >
              ...
              {isDateMenuOpen && (
                <div className="absolute top-full mt-2 left-0 right-0 bg-white/95 dark:bg-neutral-900/95 backdrop-blur-xl border border-neutral-200/60 dark:border-neutral-800/60 rounded-2xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                  <div className="max-h-52 overflow-y-auto overscroll-contain p-2 scrollbar-thin scrollbar-thumb-emerald-200 dark:scrollbar-thumb-emerald-900/50">
                    {fechasDisponibles.map((f, i) => (
                      <button
                        ...
                        className={`w-full flex items-center justify-between text-left px-4 py-3 rounded-xl text-sm font-medium transition-all ${fechaSeleccionada === f.fecha ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-bold' : 'text-neutral-700 dark:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-neutral-800'}`}
                      >
                        {f.etiqueta}
            ...
            <button
              ...
              className="px-6 py-3 sm:py-3.5 rounded-2xl font-bold text-sm shadow-sm transition-all bg-neutral-900 hover:bg-black dark:bg-white dark:hover:bg-neutral-100 text-white dark:text-black disabled:opacity-50 disabled:pointer-events-none hover:shadow-lg active:scale-95 flex items-center justify-center gap-2 h-[46px] sm:h-[52px] shrink-0"
            >
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-3 w-full lg:w-1/2">
              <div className="relative flex-1 min-w-0">
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between px-4 py-3 sm:py-3.5 rounded-2xl text-sm font-semibold transition-all duration-200 bg-neutral-50 dark:bg-neutral-950 border ${isEmpMenuOpen ? 'border-emerald-500 ring-2 ring-emerald-500/20' : 'border-neutral-200/60 dark:border-neutral-800/50 hover:border-neutral-300 dark:hover:border-neutral-700'}`}
                >
                ...
                {isEmpMenuOpen && (
                  <div className="absolute top-full mt-2 left-0 right-0 lg:right-auto lg:w-80 bg-white/95 dark:bg-neutral-900/95 backdrop-blur-xl border border-neutral-200/60 dark:border-neutral-800/60 rounded-2xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                    <div className="max-h-52 overflow-y-auto overscroll-contain p-2 scrollbar-thin scrollbar-thumb-emerald-200 dark:scrollbar-thumb-emerald-900/50">
                      {empleadosParaAgregar.length === 0 ? (
                        ...
                        empleadosParaAgregar.map(emp => (
                          <button
                            ...
                            className={`w-full text-left px-4 py-3 rounded-xl transition-all text-sm ${extraId === emp.id.toString() ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-800 dark:text-neutral-200'}`}
                          >
              ...
              <button
                ...
                disabled={!extraId}
                className="p-3 sm:p-3.5 rounded-2xl bg-neutral-50 dark:bg-neutral-950 text-neutral-600 dark:text-neutral-400 border border-neutral-200/60 dark:border-neutral-800/50 hover:border-emerald-500 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 disabled:opacity-40 transition-all active:scale-95 h-[46px] sm:h-[52px] w-[46px] sm:w-[52px] flex items-center justify-center shrink-0"
              >
                <UserPlus size={20} />
              </button>
            </div>
      ...
      <div className="flex-1 min-h-0 flex flex-col w-full">
        <TablaNominaReactiva
          ...
          onRemove={quitarEmpleado} // 🚀 Pasamos la función
