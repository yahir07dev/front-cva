...
export default function TablaNominaReactiva({ 
  ...
  return (
    <>
      <div className={`flex flex-col w-full rounded-2xl sm:rounded-3xl overflow-hidden transition-all duration-300 bg-white/70 dark:bg-neutral-900/70 backdrop-blur-sm border border-neutral-200/40 dark:border-neutral-800/50 shadow-xl shadow-black/5 dark:shadow-black/30 h-full min-h-[400px] ${isReadOnly ? 'opacity-90' : ''}`}>
                  ...
                  <tr key={renglon.empleado_id} className={`group transition-all duration-200 bg-white/40 dark:bg-neutral-950/40 hover:bg-white/70 dark:hover:bg-neutral-900/60 ${!isReadOnly ? 'hover:shadow-md hover:scale-[1.002]' : ''} rounded-xl sm:rounded-2xl w-full`}>
                      ...
                      <div className="flex items-center gap-2 sm:gap-3">
                        <div className="relative w-8 h-8 sm:w-10 sm:h-10 md:w-11 md:h-11 rounded-full overflow-hidden ring-1 ring-emerald-200/50 dark:ring-emerald-900/40 shadow-sm shrink-0">
                          {renglon.foto_perfil_url ? (
                            <img src={renglon.foto_perfil_url} alt={renglon.nombre_completo} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                          ) : (
                            <div className="w-full h-full bg-gradient-to-br from-emerald-100 to-emerald-50 dark:from-emerald-950 dark:to-neutral-900 flex items-center justify-center"><UserRound size={14} className="text-emerald-600/70" /></div>
                          )}
                        </div>
                        <div className="min-w-0 flex-1">
                          <div className="font-semibold text-neutral-900 dark:text-neutral-100 text-xs sm:text-sm leading-tight truncate">{renglon.nombre_completo}</div>
                          <div className="text-[9px] sm:text-[10px] text-neutral-500 dark:text-neutral-400 mt-0.5 truncate">Base: {formatMoney(renglon.sueldo_base)}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="p-2.5 sm:p-3 md:p-4 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-1.5 sm:gap-2 font-bold text-neutral-800 dark:text-neutral-100">
                        {!isReadOnly && (
                          <button onClick={() => setEmpleadoCalculadora(renglon)} className="p-1.5 sm:p-2 rounded-lg sm:rounded-xl bg-white/80 dark:bg-neutral-900/60 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 text-neutral-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors border border-neutral-200/50 dark:border-neutral-800/50"><Calculator size={13} /></button>
                        )}
          ...
          <div className="overflow-x-auto w-full md:w-auto flex-1">
            <div className="flex gap-4 sm:gap-6 text-sm font-medium min-w-max justify-start md:justify-end">
              ...
              <div className="flex flex-col items-end pl-4 border-l border-emerald-500/30 shrink-0">
                <span className="text-[9px] text-emerald-700 dark:text-emerald-400 uppercase font-bold">Efectivo Total</span>
                <span className="text-xl sm:text-2xl font-black text-emerald-700 dark:text-emerald-400 tabular-nums">{formatMoney(totales.pagoNetoEfectivo)}</span>
              </div>
            </div>
          </div>
