...
const PrestamoCard = ({ prestamo, canManage, onOpenAbono, onTogglePausa }: PrestamoCardProps) => {
  ...
  return (
    <div className={`group relative w-full bg-white dark:bg-neutral-900/60 border border-neutral-100 dark:border-white/[0.06] rounded-[28px] overflow-hidden shadow-[0_2px_16px_rgba(0,0,0,0.04)] dark:shadow-[0_2px_24px_rgba(0,0,0,0.3)] transition-all duration-500 ease-[cubic-bezier(.16,1,.3,1)] hover:shadow-[0_8px_40px_rgba(0,0,0,0.08)] dark:hover:shadow-[0_8px_40px_rgba(0,0,0,0.5)] hover:-translate-y-1 ${isCompleted ? 'opacity-75 hover:opacity-100' : ''}`}>
      
      <div className="absolute top-0 left-0 right-0 h-[2px]" style={{ background: stateColor.accent, opacity: 0.7 }} />
      ...
      <div className="relative z-10 p-5 md:p-6 flex flex-col lg:flex-row lg:items-center gap-5 lg:gap-6">
        ...
        <div className="flex items-center gap-4 lg:w-[220px] shrink-0">
          <div className="relative shrink-0">
            <div className={`relative h-[52px] w-[52px] rounded-full overflow-hidden ring-2 ring-offset-2 ring-offset-white dark:ring-offset-[#0f0f12] transition-all duration-500 group-hover:scale-[1.06]`} style={{ ringColor: stateColor.accent } as any}>
              {emp?.foto_perfil_url ? (
                <img src={emp.foto_perfil_url} alt="Avatar" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
              ) : (
                <div className="h-full w-full flex items-center justify-center text-white font-black text-base uppercase" style={{ background: stateColor.accent }}>
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-1">
              <span className="inline-block h-1.5 w-1.5 rounded-full shrink-0" style={{ background: stateColor.accent }} />
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div className="flex items-start gap-6">
            <div className="group/stat">
              ...
              <p className="text-xl font-black text-neutral-900 dark:text-white tabular-nums tracking-tight leading-none">{formatMoney(total)}</p>
            </div>
            ...
            <div>
              ...
              <p className="text-xl font-black tabular-nums tracking-tight leading-none" style={{ color: stateColor.accent }}>
            ...
            <div>
              ...
              <p className="text-xl font-black text-neutral-700 dark:text-neutral-300 tabular-nums tracking-tight leading-none">{formatMoney(pagado)}</p>
            </div>
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-xs font-black tabular-nums" style={{ color: stateColor.accent }}>{porcentaje}%</span>
            </div>
            <div className="relative h-2 w-full rounded-full bg-neutral-100 dark:bg-white/[0.05] overflow-visible">
              <div className="absolute top-0 left-0 h-full rounded-full transition-all duration-1000 ease-[cubic-bezier(.16,1,.3,1)]" style={{ width: `${porcentaje}%`, background: stateColor.bar, boxShadow: `0 0 10px ${stateColor.accent}66` }}>
              ...
              {porcentaje > 0 && porcentaje < 100 && (
                <div className="absolute top-1/2 -translate-y-1/2 h-3.5 w-3.5 rounded-full border-2 border-white dark:border-[#0f0f12] shadow-md transition-all duration-1000 ease-[cubic-bezier(.16,1,.3,1)]" style={{ left: `calc(${porcentaje}% - 7px)`, background: stateColor.accent, boxShadow: `0 0 8px ${stateColor.accent}aa` }} />
              )}
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-row lg:flex-col items-center justify-between lg:justify-end gap-3 lg:w-[180px] shrink-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-[0.06em]" style={{ background: stateColor.badge, color: stateColor.badgeText }}>
              {prestamo.estado}
            </div>
          </div>
          ...
          {canManage && !isCompleted && (
            <div className="flex gap-2 w-full lg:w-full">
              <button
                ...
                title={isPaused ? 'Reanudar cobro' : 'Pausar cobro'}
                ...
                style={{
                  ...
                  background: isPaused ? `${stateColor.accent}15` : pauseHover ? `${stateColor.accent}0e` : 'rgba(0,0,0,0.03)',
                }}
              >
                {isPaused ? <PlayCircle size={18} style={{ color: stateColor.accent }} className="relative z-10 transition-transform duration-300" /> : <PauseCircle size={18} className="relative z-10 transition-all duration-300" style={{ color: pauseHover ? stateColor.accent : 'rgba(0,0,0,0.35)' }} />}
              </button>
              ...
              >
                <span>Abonar</span>
                <ChevronRight size={15} className="transition-transform duration-300" style={{ transform: abonoHover ? 'translateX(3px)' : 'translateX(0)' }} />
              </button>
            </div>
          )}
        </div>
      </div>
      <style jsx>{`@keyframes shimmer { 0% { transform: translateX(-100%); } 100% { transform: translateX(400%); } }`}</style>
    </div>
  )
}

// React.memo previene que esta tarjeta se re-renderice si el 'prestamo' no ha cambiado.
export default React.memo(PrestamoCard)