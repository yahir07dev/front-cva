...
import { crearRol, actualizarRol } from '@/src/services/rolesService'

export default function RolesClient() {
  const { roles, permisosCatalogo, loading, isSuperAdmin, recargar } = useRoles(true)
  ...
  const handleGuardar = async () => {
    ...
    setIsSaving(true)
    try {
      if (rolId) { await actualizarRol(rolId, { nombre, descripcion }, selectedPerms) }
      else { await crearRol({ nombre, descripcion }, selectedPerms) }
  ...
  if (loading && roles.length === 0) {
    return (
      <div className="flex h-full items-center justify-center bg-white dark:bg-[#0a0a0a]">
  ...
  if (!isSuperAdmin) {
    return (
      <div className="flex h-full items-center justify-center bg-white dark:bg-[#0a0a0a] text-neutral-400 text-sm">
        Sin permisos para gestionar roles.
      </div>
    )
  }
      ...
      <div className="roles-root relative flex flex-col h-full w-full bg-[#fafafa] dark:bg-[#0a0a0a] overflow-hidden">
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
                      rol.es_sistema
                        ? 'bg-amber-50 dark:bg-amber-400/10 text-amber-500 dark:text-amber-400'
                        : 'bg-indigo-50 dark:bg-indigo-400/10 text-indigo-500 dark:text-indigo-400'
                    }`}>
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity ml-2 shrink-0">
                    <button
                      ...
                      className="w-7 h-7 flex items-center justify-center text-neutral-400 hover:text-indigo-500 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 rounded-lg transition-colors"
                    >
                      <Edit2 size={13} />
                    </button>
                    {!rol.es_sistema && (
                      <button className="w-7 h-7 flex items-center justify-center text-neutral-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-lg transition-colors">
                        <Trash2 size={13} />
                      </button>
                    )}
                  </div>
        ...
        {modalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            {/* Backdrop */}
            <div
              className="absolute inset-0 bg-black/40 dark:bg-black/60 backdrop-blur-[6px]"
            ...
            <div className="modal-panel relative w-full max-w-2xl max-h-[88vh] flex flex-col bg-white dark:bg-[#111111] rounded-2xl shadow-2xl border border-neutral-100 dark:border-white/[0.07] overflow-hidden">
                ...
                <div>
                  ...
                  {esSistema && (
                    <p className="text-[12px] text-amber-500 dark:text-amber-400 mt-0.5">
                      Rol de sistema — nombre protegido
                    </p>
                  )}
                </div>
                <button
                  ...
                  disabled={isSaving}
                  className="w-8 h-8 flex items-center justify-center text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-white/[0.07] rounded-lg transition-colors disabled:opacity-40"
                >
                  <X size={16} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto scrollbar-none p-6 space-y-7">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div>
                    <label className="block text-[12px] font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider mb-2">
                      Nombre del rol
                    </label>
                    <input
                      type="text"
                      ...
                      className="w-full px-4 py-3 text-[14px] bg-neutral-50 dark:bg-white/[0.04] border border-neutral-200 dark:border-white/[0.08] rounded-xl outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-400 dark:focus:border-indigo-500/50 transition-all disabled:opacity-50 text-neutral-900 dark:text-white placeholder-neutral-400 dark:placeholder-neutral-600"
                    />
                  </div>
                  <div>
                    <label className="block text-[12px] font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider mb-2">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-[12px] font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">
                      Permisos asignados
                    </span>
                    {selectedPerms.length > 0 && (
                      <span className="ml-auto tag-pill bg-indigo-100 dark:bg-indigo-500/15 text-indigo-600 dark:text-indigo-400 px-2 py-0.5 rounded-full font-medium">
                      ...
                      {Object.entries(permisosPorModulo).map(([modulo, permisos]) => (
                        <div key={modulo} className="rounded-xl border border-neutral-100 dark:border-white/[0.06] overflow-hidden">
                          <div className="px-4 py-2.5 bg-neutral-50 dark:bg-white/[0.03] border-b border-neutral-100 dark:border-white/[0.06]">
                            <span className="tag-pill font-semibold text-neutral-500 dark:text-neutral-400 uppercase">
                              {modulo.replace('-', ' ')}
                            </span>
                          </div>
              ...
              <div className="px-6 py-4 border-t border-neutral-100 dark:border-white/[0.06] flex items-center justify-between shrink-0 bg-white dark:bg-[#111111]">
                <button
                  onClick={() => setModalOpen(false)}
                  disabled={isSaving}
                  className="text-[13px] font-medium text-neutral-500 dark:text-neutral-500 hover:text-neutral-800 dark:hover:text-neutral-300 transition-colors px-3 py-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-white/[0.05] disabled:opacity-40"
                >
                  Cancelar
                </button>
                <button
                  ...
                  disabled={isSaving}
                  className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white text-[13px] font-semibold px-5 py-2.5 rounded-xl transition-colors shadow-md shadow-indigo-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isSaving
